...
parameters {
  //real<lower=0> NEP_mu;
  real<lower=-1000, upper = 1000> NEP[n_ParamEst];
  //real<lower=0> SED1_mu;
  real<lower=0, upper=1> SED1[n_ParamEst];
  //real<lower=0> MIN_mu;
  real<lower=300, upper=500> MIN[n_ParamEst];
  //real<lower=0> SED2_mu;
  real<lower=2200, upper=3200> SED2[n_ParamEst];
}
transformed parameters {
  ...
  if (stratified[i] == 0) {
    ...
    SED1[i_Param[i]] *  (DO_tot[i-1]/(khalf + DO_tot[i-1])) * theta0[i-1] * area_epi[i-1]/volume_tot[i-1] +
    k600t[i-1]  *  (o2satt[i-1] - DO_tot[i-1])  * area_epi[i-1]/volume_tot[i-1] - ///25. -
    MIN[i_Param[i]] * (DO_tot[i-1]/(khalf + DO_tot[i-1])) * theta0[i-1];
	...
	if(first_day == 1) {
	    ...
	    dDOdt_hyp[i] =  - MIN[i_Param[i]] * (DO_hyp[i-1]/(khalf + DO_hyp[i-1])) * theta2[i-1] -
  ...
  for(i in 1:n_ParamEst){
   if (i==1){
      NEP[i] ~ normal(500,100); // Use i+1 because initial value is the first in the vector
   	  MIN[i] ~ normal(400,0.1);
      SED1[i] ~ normal(1.,0.1);
      SED2[i] ~ normal(2700,100);
   } else {
      NEP[i] ~ normal(NEP[i-1],100); // Use i+1 because initial value is the first in the vector
...
generated quantities {
  ...
  for (i in 2:d){
    ...
    if (stratified[i] == 0) {
      ...
      Fsed1[i] = -SED1[i_Param[i]] *  (DO_tot[i-1]/(khalf + DO_tot[i-1])) * theta0[i-1] * area_epi[i-1]/volume_tot[i-1];
      Fatm[i] = k600t[i-1] * (o2satt[i-1] - DO_tot[i-1]) * area_epi[i-1]/volume_tot[i-1];
      Fmin[i] =  - MIN[i_Param[i]] * (DO_tot[i-1]/(khalf + DO_tot[i-1])) * theta0[i-1];
      Fsed2[i] = 0;
    } else {
        first_day2 = 0;
          for (k in 1:len_strat_pos){
      ...
      if (first_day2 == 0){
        ...
        Fmin[i] =  - MIN[i_Param[i]] * (DO_hyp[i-1]/(khalf + DO_hyp[i-1])) * theta2[i-1];
